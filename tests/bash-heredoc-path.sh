...
truss -f -t pipe,write,open,openat,unlink,unlinkat -o $T/tr "$B" $T/s.sh > $T/out 2>/dev/null
[ "$(tr -d " " < $T/out)" = 5800 ] || { echo "  $B did not run the here-document (not a bash binary?)"; exit 2; }
