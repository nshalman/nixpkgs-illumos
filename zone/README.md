...
| `/etc/mail`, `/etc/mailer.conf`, `/var/mail`, `/var/spool/{client,}mqueue` | local mail for root and the zone's users: sendmail's configuration made from the gate's m4 sources, the platform's sendmail and `mail.local` delivering to `/var/mail`; the `smmsp` account; `localhost.local` on the `127.0.0.1` line of `/etc/inet/hosts`, so sendmail can qualify its name | the zone image |
...
| `smf-seed.nix`, `smf-seed-archive.xml` | a seed `/etc/svc/repository.db` from upstream illumos-gate manifests: the gate's non-global seed services and sendmail's two (their manifests shipped in `/var/svc/manifest/network`, so the first boot's `generic.xml` enables them), just enough for early manifest import to load the platform's manifests from `/lib/svc/manifest` on first boot, before any service starts. The build checks `svccfg archive` of the result against `smf-seed-archive.xml` |
